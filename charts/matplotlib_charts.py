...
class ROCCurveChart(QWidget):
    ...
    def _draw(self):
        ...
        fpr = data.ROC_FPR
        tpr = data.ROC_TPR
        ax.plot(fpr, tpr, color=CYAN, linewidth=2.2,
                label=f"AUC = {data.MODEL_METRICS['roc_auc']:.1f}%")
...
class PRCurveChart(QWidget):
    ...
    def _draw(self):
        ...
        precision = np.clip(
            1 - recall * 0.35 + np.random.uniform(-0.03, 0.03, 50), 0, 1
        )
        precision = np.sort(precision)[::-1]
        ax.plot(recall, precision, color=PURPLE, linewidth=2.2, label="P-R Curve")
        ax.fill_between(recall, precision, alpha=0.15, color=PURPLE)
